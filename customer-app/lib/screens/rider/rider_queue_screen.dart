...
class _RiderQueueScreenState extends State<RiderQueueScreen> with SingleTickerProviderStateMixin {
  ...
  // Pickup: "today" = confirmed & created today; "all" = every pending pickup.
  // Delivery: "today" = ready to hand over (packaged / out); "all" = delivered.
  ...
  Future<void> _advance(AdminOrder o) async {
    ...
    try {
      ...
      setState(() {}); // re-filter (the order may now leave this tab)
    } catch (e) {
      if (!mounted) return;
    ...
    return Scaffold(
      ...
      body: _loading
          ...
          : _error != null
              ...
              : TabBarView(
                  controller: _tab,
                  children: [_list(_today, emptyToday: true), _list(_all, emptyToday: false)],
                ),
    );
  }

  Widget _list(List<AdminOrder> orders, {required bool emptyToday}) {
                ...
                Padding(
                  ...
                  child: Column(
                    children: [
                      ...
                      Text(
                        _isPickup
                            ? AppLanguage.tr('কোনো পিকআপ নেই')
                            : AppLanguage.tr('কোনো ডেলিভারি নেই'),
...
class _RiderCollectionScreenState extends State<RiderCollectionScreen> with SingleTickerProviderStateMixin {
  ...
  List<AdminOrder> get _todayDelivered =>
      _delivered.where((o) => _isToday(o.deliveredAt)).toList();

  int get _todayTotal => _todayDelivered.fold(0, (s, o) => s + o.total);

  /// Delivered orders grouped by delivery day, newest day first.
