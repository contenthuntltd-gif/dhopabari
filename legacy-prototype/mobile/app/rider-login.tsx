...
export default function RiderLoginScreen() {
  ...
  const handleLogin = async () => {
    ...
    setErrorMsg('');
    try {
      const res = await apiService.riderLogin({
        username: username.trim().toLowerCase(),
        password: password.trim()
      });
      if (res.ok) {
        await AsyncStorage.setItem('rider_session_id', res.riderId);
        await AsyncStorage.setItem('rider_session_name', res.name);
        router.replace({
          pathname: '/rider-dashboard',
          params: { riderId: res.riderId }
        });
      } else {
        setErrorMsg(res.error || (lang === 'bn' ? 'ভুল ইউজারনেম অথবা পাসওয়ার্ড' : 'Invalid credentials'));
      }
    } catch (e: any) {
      // Local fallback for offline/prototype mode robustness
      const cleanUser = username.trim().toLowerCase();
      const cleanPass = password.trim();
      const mockRiders: Record<string, string> = { karim: 'rider_karim', mamun: 'rider_mamun', shahin: 'rider_shahin' };
      if (mockRiders[cleanUser] && cleanPass === '123') {
        const id = mockRiders[cleanUser];
        const names: Record<string, string> = { rider_karim: 'করিম ভাই', rider_mamun: 'মামুন ভাই', rider_shahin: 'শাহিন ভাই' };
        await AsyncStorage.setItem('rider_session_id', id);
        await AsyncStorage.setItem('rider_session_name', names[id]);
        router.replace({
          pathname: '/rider-dashboard',
          params: { riderId: id }
        });
    ...
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.heroContainer}>
          <LinearGradient colors={['#0d9488', '#0f766e']} style={styles.hero}>
            ...
            <View style={styles.headerBar}>
              ...
              <Text style={styles.headerTitle}>{lang === 'bn' ? 'রাইডার পোর্টাল' : 'Rider Portal'}</Text>
              <View style={{ width: 36 }} />
            </View>
        ...
        <View style={styles.form}>
          <Text style={styles.title}>{lang === 'bn' ? 'রাইডার লগইন' : 'Rider Sign In'}</Text>
          <Text style={styles.subtitle}>{lang === 'bn' ? 'আপনার রাইডার প্যানেল এক্সেস করুন' : 'Access your rider panel dashboard'}</Text>
          ...
          <View style={styles.field}>
            ...
            <TextInput
              ...
              placeholder={lang === 'bn' ? 'ইউজারনেম (যেমন: karim)' : 'Username (e.g. karim)'}
          ...
          <View style={styles.field}>
            ...
            <TextInput
              ...
              placeholder={lang === 'bn' ? 'পাসওয়ার্ড (যেমন: 123)' : 'Password (e.g. 123)'}
              placeholderTextColor="#a2abb8"
              secureTextEntry
              autoCapitalize="none"
            />
          ...
          <TouchableOpacity onPress={() => router.push('/login')} style={{ marginTop: 20 }}>
            <Text style={styles.customerLinkText}>
              ...
              <Text style={{ color: '#0d9488', fontWeight: '900' }}>{lang === 'bn' ? 'এখানে যান' : 'Go here'}</Text>
            </Text>
          </TouchableOpacity>
        </View>
